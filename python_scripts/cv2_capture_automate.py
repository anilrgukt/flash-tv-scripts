...
def find_matching_face(target_embedding, current_frame_img, dboxes, threshold=0.436, use_recognition=True):
    ...
    if not use_recognition or frame_skip_counter % face_recognition_interval != 0:
        ...
        if last_matched_face is not None:
            for dbox in dboxes:
                if (dbox["prob"] > 0.1 and area(dbox) > 35.00 and 
                    abs(dbox["left"] - last_matched_face["left"]) < 15 and
                    abs(dbox["top"] - last_matched_face["top"]) < 15):
                    return dbox
        return None
    ...
    sorted_faces = sorted([d for d in dboxes if d["prob"] > 0.1 and area(d) > 35.00], 
                         key=lambda x: x["prob"], reverse=True)[:3]  # Only check top 3 faces
...
def save_selected_face(img, face_num):
    ...
    if matched_face is None:
        for detface in current_faces:
            if (area(detface) > 35.00 and detface["prob"] >= 0.11 and 
                abs(detface["left"] - selected_face_box["left"]) < 15 and
                abs(detface["top"] - selected_face_box["top"]) < 15):
...
# Initialize face verification model with proper error handling
try:
    import torch
...
selected_face_box = None
selected_face_data = None
selected_face_embedding = None  # Store embedding of selected face
current_faces = []
current_frame_img = None  # Store current frame for face recognition
face_collection = {
...
face_recognition_interval = 3  # Run face recognition every 3 frames
face_embedding_cache = {}  # Cache embeddings to avoid recomputation
last_matched_face = None  # Cache last matched face for position fallback
...
while True:
    if not q.empty():
        ...
        # BACKUP SAVING: Continuously save the currently selected face using face recognition
        face_lost = False  # Track if face is lost
        if show_face is not None and selected_face_box is not None:
            matched_face = None
            ...
            if matched_face is not None:
                ...
                backup_output_path = os.path.join(imsave_dir, show_face.lower(), str(c).zfill(6) + "_backup_" + str(sub_count[show_face]) + ".png")
