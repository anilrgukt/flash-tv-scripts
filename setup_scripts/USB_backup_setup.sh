...
if borg list > /dev/null 2>&1 && borg list | grep -Pq "\d{3}008" && [ "$skip_checking" -eq 1 ]; then
...
if [ ! `lsusb | grep -q "SanDisk Corp. Ultra Fit"` ]; then
	...
	# Enable automounting of the USB on boot (disabled by default)
 	FSTAB=/etc/fstab
  	...
  	backup_usb_mount_path=/media/flashsys008/${backup_usb_uuid}

 	backup_usb_fstab_line="UUID=${backup_usb_uuid} /media/flashsys008/${backup_usb_uuid} auto uid=${UID},gid=${UID} 0 0"

 	grep -q '.*UUID=.* /media/flashsys008/.* auto uid=.*,gid=.* 0 0.*' "${FSTAB}" || echo "${backup_usb_fstab_line}" | sudo tee -a "${FSTAB}"
  	sudo sed -i "s@.*UUID=.* /media/flashsys008/.* auto uid=.*,gid=.* 0 0.*@${backup_usb_fstab_line}@" "${FSTAB}"
	...
	encoded_password=`python3 /home/flashsys008/flash-tv-scripts/python_scripts/check_and_encode_password.py "${temp_file}"`
	exit_code=$?
	...
	BASHRC=/home/flashsys008/.bashrc
	...
	export BORG_REPO=${backup_usb_mount_path}/USB_Backup_Data_flashsys008
	
	BORG_REPO_EXPORT_LINE="export BORG_REPO='${backup_usb_mount_path}/USB_Backup_Data_flashsys008'"
	...
	borg key export --paper :: > ${backup_usb_mount_path}/borg-encrypted-key-backup-flashsys008.txt
	borg key export --paper :: > /home/flashsys008/borg-encrypted-key-backup-flashsys008.txt
	borg key export --paper :: > /home/flashsys008/flash-tv-scripts/setup_scripts/borg-encrypted-key-backup-flashsys008.txt
